...
/*                            empiler
     ...
     Sortie : pile avec l'élement ajouté au sommet
...
pile_t * inversion(pile_t * pile, int * codeErreur){
     ...
     if (file){													// Si l'allocation OK :
          while(!estVideP(pile)){										// Pour chaque élément de la pile
               enfiler(file, depiler(pile));							// Ajouter dans la file de l'élément au sommet
																// de la file (élément dépilé de la pile)
          }
          afficherF(file);											// Affichage de la files
          while(!estVideF(file)){										// Tant que la file est NON vide
               empiler(pile,defiler(file));							// Ajout dans la pile de l'élément en tête de la file
															     // Et suppression de cet élément dans la file
          }
     }
     else{                                                                      // Si alloc file échouée :
